...
- В публичном профиле появился честный online-индикатор и блок «Смотрит сейчас» прямо под уровнем и статистикой.
- «Смотрит сейчас» показывается только пока пользователь онлайн и у него реально открыт плеер; устаревшая серия исчезает.
- Счётчик карточки теперь показывает уникальных владельцев, а не число открытий/повторов.
- Мобильная прокрутка Media Deck и исправления CAPTCHA из V25.9.9 сохранены.

Senime V25.9.9 — Mobile CAPTCHA + Player Deck

- Исправлена регистрация и восстановление пароля: CAPTCHA-токен снова отправляется в Supabase Auth.
- На экране создаётся только одна CAPTCHA для открытой вкладки; на узких телефонах используется compact-режим и автоматический retry.
- На телефоне Media Deck теперь находится сразу под плеером, перед комментариями, и нормально прокручивается пальцем.
- Серии и дорожки получили крупные touch-зоны; сетка серий адаптируется до четырёх колонок.
- Главное боковое меню тоже прокручивается на низких экранах телефона.

Senime V25.9.8 — Kodik Stale Episode Link Fix

- Старые сохранённые ссылки Kodik теперь автоматически признаются недействительными и обновляются для выбранной серии.
- Каждая Kodik-дорожка хранит номер серии и сезона, для которых она была реально получена.
- Добавлен официальный параметр `hide_resume_button=true`; селекторы скрываются официальным `hide_selectors=true`, без обрезания iframe.
