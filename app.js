...
/* ==========================================================================
   SENIME V24.7 · COMMUNITY & COLLECTION UPDATE
   ========================================================================== */
const SENIME_V247='24.7';
function senimeSignedV247(){return !!accountSessionV23?.access_token&&!!(accountSessionV23?.user?.id||socialMeV241?.())}
function senimeMeV247(){return accountSessionV23?.user?.id||socialMeV241?.()||null}
async function senimeFetchV247(path,{method='GET',body=null,auth=true,prefer=''}={}){
  const c=accountConfigV23();if(!accountConfiguredV23())throw new Error('Senime Cloud не подключён.');
  const token=auth?(accountSessionV23?.access_token||c.key):c.key,headers=accountAuthHeadersV23(token),opts={method,headers};
  if(prefer)headers.Prefer=prefer;if(body!==null){headers['Content-Type']='application/json';opts.body=JSON.stringify(body)}
  return accountJsonFetchV23(`${c.url}/rest/v1/${path}`,opts);
}
async function senimeRpcV247(name,body={},auth=true){return senimeFetchV247(`rpc/${name}`,{method:'POST',body,auth})}
const senimeDelayV247=ms=>new Promise(r=>setTimeout(r,ms));
function attrV247(s){return esc(String(s??'')).replace(/"/g,'&quot;').replace(/'/g,'&#39;')}

/* Keep enough public stats for fair leaderboards. */
const socialBuildPublicPayloadBeforeV247=socialBuildPublicPayloadV242;
socialBuildPublicPayloadV242=function(){
  const out=socialBuildPublicPayloadBeforeV247(),p=ensureProfileV16();let seconds=0;
  try{for(const rec of Object.values(p.episodeLedger||{})){if(!rec?.verified)continue;const w=Number(watchedSecondsV16?.(rec)||0),d=Number(rec.duration||0);seconds+=Math.max(0,Math.min(d||w,w||d))}}catch{}
  out.watchSeconds=Math.round(seconds);out.collectionCount=(p.collection||[]).length;out.completed=Number(latestData.sections?.completed?.length||0);out.senimeVersion=SENIME_V247;return out;
};

const communityV247={tab:'chats',selectedChat:null,messages:[],reactions:new Map(),replyTo:null,editing:null,chatSearch:'',notifications:[],leaderMetric:'level'};
function ensureCommunityModalV247(){
  if($('#communityModalV247'))return;
  document.body.insertAdjacentHTML('beforeend',`<div id="communityModalV247" class="community-modal-v247 hidden"><div class="community-shell-v247"><header class="community-head-v247"><div class="community-brand-v247"><span>◈</span><div><b>Senime Community</b><small>люди · сообщения · рейтинги · подборки</small></div></div><nav id="communityTabsV247" class="community-tabs-v247"><button data-community-tab="chats">💬 Чаты</button><button data-community-tab="friends">👥 Друзья</button><button data-community-tab="notifications">🔔 Уведомления <i id="communityNotificationCountV247"></i></button><button data-community-tab="leaderboards">🏆 Лидерборды</button><button data-community-tab="collections">📚 Подборки</button></nav><button id="communityCloseV247" class="community-close-v247">✕</button></header><main class="community-body-v247"><section id="communityChatsV247" class="community-pane-v247"></section><section id="communityFriendsV247" class="community-pane-v247"></section><section id="communityNotificationsV247" class="community-pane-v247"></section><section id="communityLeaderboardsV247" class="community-pane-v247"></section><section id="communityCollectionsV247" class="community-pane-v247"></section></main></div></div>`);
  $('#communityCloseV247').onclick=closeCommunityV247;$('#communityModalV247').onclick=e=>{if(e.target===e.currentTarget)closeCommunityV247()};
  $('#communityTabsV247').onclick=e=>{const b=e.target.closest('[data-community-tab]');if(b)setCommunityTabV247(b.dataset.communityTab)};
}
function openCommunityV247(tab='chats'){
  ensureCommunityModalV247();if(['chats','friends','notifications'].includes(tab)&&!senimeSignedV247()){openAccountV23();return}
  $('#communityModalV247').classList.remove('hidden');document.body.style.overflow='hidden';setCommunityTabV247(tab);
}
function closeCommunityV247(){$('#communityModalV247')?.classList.add('hidden');if($('#profileModal')?.classList.contains('hidden')&&$('#watchModal')?.classList.contains('hidden'))document.body.style.overflow=''}
async function setCommunityTabV247(tab){
  communityV247.tab=tab;document.querySelectorAll('#communityTabsV247 [data-community-tab]').forEach(b=>b.classList.toggle('active',b.dataset.communityTab===tab));document.querySelectorAll('.community-pane-v247').forEach(p=>p.classList.remove('active'));
  const pane=$(`#community${tab[0].toUpperCase()+tab.slice(1)}V247`);pane?.classList.add('active');
  if(tab==='chats')await loadCommunityChatsV247();if(tab==='friends')await loadCommunityFriendsV247();if(tab==='notifications')await loadNotificationsV247(true);if(tab==='leaderboards')await loadLeaderboardV247(communityV247.leaderMetric);if(tab==='collections')await loadCollectionsHubV247();
}
Object.assign(window,{openCommunityV247,closeCommunityV247,setCommunityTabV247});

function bindSidebarCommunityV247(){
  $('#sidebarChatsV247')?.addEventListener('click',()=>openCommunityV247('chats'));$('#sidebarFriendsV247')?.addEventListener('click',()=>openCommunityV247('friends'));$('#sidebarNotificationsV247')?.addEventListener('click',()=>openCommunityV247('notifications'));$('#sidebarLeaderboardsV247')?.addEventListener('click',()=>openCommunityV247('leaderboards'));$('#sidebarCollectionsV247')?.addEventListener('click',()=>openCommunityV247('collections'));
}
bindSidebarCommunityV247();
async function openUserProfileV247(id){if(!id)return;if(String(id)===String(senimeMeV247())){closeCommunityV247();openProfileV16();setProfileTabV16('overview');return}closeCommunityV247();await socialOpenPublicProfileV242(id)}
window.openUserProfileV247=openUserProfileV247;

/* Friends page */
async function communitySearchPeopleV247(){
  const raw=String($('#communityFriendSearchV247')?.value||'').trim().replace(/^@/,''),out=$('#communityFriendResultsV247');if(!out)return;if(raw.length<2){out.innerHTML='<div class="community-empty-v247">Введи хотя бы 2 символа.</div>';return}
  try{const clean=raw.replace(/[,*()]/g,''),q=new URLSearchParams();q.set('or',`(display_name.ilike.*${clean}*,handle.ilike.*${clean}*)`);q.set('limit','30');const rows=(await socialFetchProfilesV242(q.toString())||[]).filter(x=>x.id!==senimeMeV247());for(const p of rows)socialStateV241.profiles.set(p.id,p);out.innerHTML=rows.length?rows.map(p=>socialPersonCardV241(p,{context:'search'})).join(''):'<div class="community-empty-v247">Никого не найдено.</div>'}catch(e){out.innerHTML=`<div class="community-empty-v247">${esc(e.message||String(e))}</div>`}
}
async function loadCommunityFriendsV247(){
  const box=$('#communityFriendsV247');if(!box)return;box.innerHTML='<div class="community-loading-v247">Загружаю друзей…</div>';
  try{await socialLoadV241();const friends=socialStateV241.friends.map(id=>socialStateV241.profiles.get(id)).filter(Boolean),req=socialStateV241.incoming.map(r=>({r,p:socialStateV241.profiles.get(r.sender_id)})).filter(x=>x.p);
    box.innerHTML=`<div class="friends-page-v247"><header class="community-page-head-v247"><div><span>PEOPLE</span><h2>Друзья и подписки</h2><p>Заявки, подписки и поиск людей — отдельно от профиля.</p></div><div class="friends-kpis-v247"><b>${friends.length}<small>друзей</small></b><b>${socialStateV241.following.length}<small>подписок</small></b><b>${socialStateV241.followers.length}<small>подписчиков</small></b></div></header><div class="community-search-v247"><input id="communityFriendSearchV247" placeholder="@handle или ник"><button id="communityFriendSearchBtnV247">Найти</button></div><div id="communityFriendResultsV247"></div>${req.length?`<section class="community-section-v247"><h3>Заявки <i>${req.length}</i></h3><div class="community-people-grid-v247">${req.map(x=>socialPersonCardV241(x.p,{context:'request',requestId:x.r.id})).join('')}</div></section>`:''}<section class="community-section-v247"><h3>Друзья <i>${friends.length}</i></h3><div class="community-people-grid-v247">${friends.length?friends.map(p=>socialPersonCardV241(p,{context:'friend'})).join(''):'<div class="community-empty-v247">Пока никого.</div>'}</div></section></div>`;
    $('#communityFriendSearchBtnV247').onclick=communitySearchPeopleV247;$('#communityFriendSearchV247').onkeydown=e=>{if(e.key==='Enter')communitySearchPeopleV247()};
  }catch(e){box.innerHTML=`<div class="community-empty-v247">${esc(e.message||String(e))}</div>`}
}

/* Telegram-like chat basics */
function chatPersonV247(id){return socialStateV241.profiles.get(id)||null}
function chatMessageV247(id){return communityV247.messages.find(x=>String(x.id)===String(id))||null}
function chatReactionGroupsV247(id){const rows=communityV247.reactions.get(String(id))||[],m=new Map(),me=senimeMeV247();for(const r of rows){const x=m.get(r.emoji)||{emoji:r.emoji,count:0,mine:false};x.count++;if(r.user_id===me)x.mine=true;m.set(r.emoji,x)}return [...m.values()]}
async function loadMessageReactionsV247(ids){communityV247.reactions=new Map();if(!ids.length)return;try{const rows=await senimeFetchV247(`message_reactions?message_id=in.(${ids.join(',')})&select=message_id,user_id,emoji`);for(const r of rows||[]){const k=String(r.message_id);if(!communityV247.reactions.has(k))communityV247.reactions.set(k,[]);communityV247.reactions.get(k).push(r)}}catch{}}
function communityRenderChatListV247(){const box=$('#communityChatListV247');if(!box)return;const friends=socialStateV241.friends.map(id=>socialStateV241.profiles.get(id)).filter(Boolean);box.innerHTML=friends.length?friends.map(p=>`<button class="community-chat-person-v247 ${communityV247.selectedChat===p.id?'active':''}" onclick="communityOpenChatV247('${p.id}')"><span class="community-chat-avatar-v247">${socialAvatarV241(p)}</span><span><b>${esc(p.display_name||p.handle||'Пользователь')}</b><small>${esc(socialHandleV241(p))}</small></span></button>`).join(''):'<div class="community-empty-v247">Добавь друзей — здесь появятся чаты.</div>'}
function chatReplyPreviewV247(m){if(!m?.reply_to)return'';const r=chatMessageV247(m.reply_to);if(!r)return'';return `<button class="chat-reply-preview-v247" onclick="communityScrollMessageV247(${Number(r.id)})"><b>${r.sender_id===senimeMeV247()?'Вы':esc(chatPersonV247(r.sender_id)?.display_name||'Сообщение')}</b><span>${esc((r.deleted_at?'Сообщение удалено':r.body||'').slice(0,90))}</span></button>`}
function communityRenderMessagesV247(){
  const box=$('#communityChatMessagesV247');if(!box)return;if(!communityV247.selectedChat){box.innerHTML='<div class="chat-zero-v247"><span>💬</span><b>Выбери диалог</b><p>Или открой друга из списка слева.</p></div>';return}
  const me=senimeMeV247(),q=communityV247.chatSearch.trim().toLowerCase();let items=communityV247.messages;if(q)items=items.filter(m=>String(m.body||'').toLowerCase().includes(q));if(!items.length){box.innerHTML=`<div class="chat-zero-v247"><span>${q?'⌕':'👋'}</span><b>${q?'Ничего не найдено':'Начало переписки'}</b></div>`;return}
  let lastDay='';box.innerHTML=items.map(m=>{const mine=m.sender_id===me,p=chatPersonV247(m.sender_id)||{},day=new Date(m.created_at).toLocaleDateString('ru-RU',{day:'numeric',month:'long'}),sep=day!==lastDay?`<div class="chat-day-v247">${esc(day)}</div>`:'';lastDay=day;const react=chatReactionGroupsV247(m.id),tools=!m.deleted_at?`<div class="chat-message-tools-v247"><button onclick="communityReplyMessageV247(${Number(m.id)})">↩</button><button onclick="communityReactMessageV247(${Number(m.id)},'❤️')">❤️</button><button onclick="communityReactMessageV247(${Number(m.id)},'🔥')">🔥</button><button onclick="communityReactMessageV247(${Number(m.id)},'😂')">😂</button><button onclick="communityCopyMessageV247(${Number(m.id)})">⧉</button>${mine?`<button onclick="communityEditMessageV247(${Number(m.id)})">✎</button><button class="danger" onclick="communityDeleteMessageV247(${Number(m.id)})">✕</button>`:''}</div>`:'';
    return `${sep}<div id="chat-message-${m.id}" class="chat-message-row-v247 ${mine?'mine':'theirs'}">${mine?'':`<button class="chat-message-avatar-v247" onclick="openUserProfileV247('${m.sender_id}')">${socialAvatarV241(p)}</button>`}<div class="chat-message-v247">${chatReplyPreviewV247(m)}${m.deleted_at?'<i>Сообщение удалено</i>':`<p>${esc(m.body||'')}</p>`}<div class="chat-message-meta-v247"><span>${new Date(m.created_at).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}${m.edited_at?' · изм.':''}</span>${mine?`<b title="${m.read_at?'Прочитано':'Доставлено'}">${m.read_at?'✓✓':'✓'}</b>`:''}</div>${react.length?`<div class="chat-reactions-v247">${react.map(r=>`<button class="${r.mine?'active':''}" onclick="communityReactMessageV247(${Number(m.id)},'${attrV247(r.emoji)}')">${esc(r.emoji)} <b>${r.count}</b></button>`).join('')}</div>`:''}${tools}</div></div>`}).join('');setTimeout(()=>{if(!q)box.scrollTop=box.scrollHeight},0);
}
async function communityOpenChatV247(id){communityV247.selectedChat=id;communityV247.replyTo=null;communityV247.editing=null;communityRenderChatListV247();const box=$('#communityChatMessagesV247');if(box)box.innerHTML='<div class="community-loading-v247">Загружаю переписку…</div>';try{const me=senimeMeV247(),q=new URLSearchParams();q.set('or',`(and(sender_id.eq.${me},receiver_id.eq.${id}),and(sender_id.eq.${id},receiver_id.eq.${me}))`);q.set('select','id,sender_id,receiver_id,body,reply_to,created_at,edited_at,deleted_at,read_at');q.set('order','created_at.asc');q.set('limit','300');communityV247.messages=await senimeFetchV247(`direct_messages?${q.toString()}`)||[];await senimeRpcV247('senime_mark_dm_read',{peer:id}).catch(()=>{});await loadMessageReactionsV247(communityV247.messages.map(m=>m.id));communityRenderMessagesV247();renderCommunityComposerStateV247()}catch(e){if(box)box.innerHTML=`<div class="community-empty-v247">${esc(e.message||String(e))}<br><small>Выполни SUPABASE_PATCH_V247.sql.</small></div>`}}
function renderCommunityComposerStateV247(){const s=$('#communityComposerStateV247'),i=$('#communityMessageInputV247');if(!s||!i)return;if(communityV247.editing){const m=chatMessageV247(communityV247.editing);s.innerHTML=`<div><b>✎ Редактирование</b><span>${esc((m?.body||'').slice(0,100))}</span></div><button onclick="communityCancelComposeModeV247()">✕</button>`;s.classList.remove('hidden');i.value=m?.body||'';return}if(communityV247.replyTo){const m=chatMessageV247(communityV247.replyTo);s.innerHTML=`<div><b>↩ Ответ</b><span>${esc((m?.body||'').slice(0,100))}</span></div><button onclick="communityCancelComposeModeV247()">✕</button>`;s.classList.remove('hidden');return}s.classList.add('hidden');s.innerHTML=''}
function communityCancelComposeModeV247(){communityV247.replyTo=null;communityV247.editing=null;const i=$('#communityMessageInputV247');if(i)i.value='';renderCommunityComposerStateV247()}
function communityReplyMessageV247(id){communityV247.replyTo=id;communityV247.editing=null;renderCommunityComposerStateV247();$('#communityMessageInputV247')?.focus()}
function communityEditMessageV247(id){communityV247.editing=id;communityV247.replyTo=null;renderCommunityComposerStateV247();$('#communityMessageInputV247')?.focus()}
async function communityDeleteMessageV247(id){try{await senimeRpcV247('senime_delete_dm',{message_id:id});await communityOpenChatV247(communityV247.selectedChat)}catch(e){profileToastV16('Не удалось удалить',e.message||String(e))}}
async function communityCopyMessageV247(id){const m=chatMessageV247(id);if(m){await copyText(m.body||'');profileToastV16('Скопировано')}}
async function communityReactMessageV247(id,emoji){try{await senimeRpcV247('senime_toggle_message_reaction',{message_id:id,reaction:emoji});await loadMessageReactionsV247(communityV247.messages.map(m=>m.id));communityRenderMessagesV247()}catch(e){profileToastV16('Реакция не отправлена',e.message||String(e))}}
function communityScrollMessageV247(id){const e=$(`#chat-message-${id}`);if(!e)return;e.scrollIntoView({behavior:'smooth',block:'center'});e.classList.add('deep-highlight-v247');setTimeout(()=>e.classList.remove('deep-highlight-v247'),1800)}
async function communitySendMessageV247(){const input=$('#communityMessageInputV247'),body=String(input?.value||'').trim(),id=communityV247.selectedChat;if(!body||!id)return;const btn=$('#communityMessageSendV247');if(btn)btn.disabled=true;try{if(communityV247.editing)await senimeRpcV247('senime_edit_dm',{message_id:communityV247.editing,message_body:body});else await senimeRpcV247('senime_send_dm',{target:id,message_body:body,reply_to:communityV247.replyTo||null});input.value='';communityV247.replyTo=null;communityV247.editing=null;await communityOpenChatV247(id)}catch(e){profileToastV16('Сообщение не отправлено',e.message||String(e))}finally{if(btn)btn.disabled=false}}
async function loadCommunityChatsV247(){const box=$('#communityChatsV247');if(!box)return;box.innerHTML='<div class="community-loading-v247">Готовлю сообщения…</div>';try{await socialLoadV241();box.innerHTML=`<div class="community-chat-page-v247"><aside class="community-chat-sidebar-v247"><header><div><span>MESSAGES</span><h2>Чаты</h2></div><button onclick="setCommunityTabV247('friends')">＋</button></header><div id="communityChatListV247"></div></aside><section class="community-chat-main-v247"><header><div><span>ЛИЧНЫЕ СООБЩЕНИЯ</span><h3>${communityV247.selectedChat?esc(chatPersonV247(communityV247.selectedChat)?.display_name||'Диалог'):'Выбери друга'}</h3></div><div class="community-chat-search-v247"><span>⌕</span><input id="communityChatSearchV247" placeholder="Поиск по переписке"></div></header><div id="communityChatMessagesV247" class="community-chat-messages-v247"></div><div id="communityComposerStateV247" class="community-composer-state-v247 hidden"></div><div class="community-composer-v247"><button id="communityEmojiV247">☺</button><textarea id="communityMessageInputV247" rows="1" maxlength="2000" placeholder="Сообщение…"></textarea><button id="communityMessageSendV247">Отправить</button></div></section></div>`;communityRenderChatListV247();communityRenderMessagesV247();$('#communityMessageSendV247').onclick=communitySendMessageV247;$('#communityMessageInputV247').onkeydown=e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();communitySendMessageV247()}};$('#communityChatSearchV247').oninput=e=>{communityV247.chatSearch=e.target.value;communityRenderMessagesV247()};$('#communityEmojiV247').onclick=()=>{const i=$('#communityMessageInputV247');if(i){i.value+=(i.value?' ':'')+'😂';i.focus()}};if(communityV247.selectedChat)await communityOpenChatV247(communityV247.selectedChat)}catch(e){box.innerHTML=`<div class="community-empty-v247">${esc(e.message||String(e))}</div>`}}
Object.assign(window,{communityOpenChatV247,communityCancelComposeModeV247,communityReplyMessageV247,communityEditMessageV247,communityDeleteMessageV247,communityCopyMessageV247,communityReactMessageV247,communityScrollMessageV247});
setInterval(()=>{if(communityV247.tab==='chats'&&communityV247.selectedChat&&!$('#communityModalV247')?.classList.contains('hidden'))communityOpenChatV247(communityV247.selectedChat)},7000);

/* Legacy social chat buttons now open the full chat UI. */
socialOpenChatV241=async function(id){communityV247.selectedChat=id;openCommunityV247('chats')};window.socialOpenChatV241=socialOpenChatV241;

/* Notifications with exact deep links. */
function notificationIconV247(k){return ({profile_comment_like:'♥',profile_comment_reply:'↩',profile_comment:'💬',anime_comment_like:'♥',anime_comment_reply:'↩',follow:'☆',friend_request:'👥',dm:'💬',message_reaction:'☺',collection_like:'📚'})[k]||'◆'}
async function loadNotificationBadgeV247(){const badge=$('#sidebarNotificationsCountV247');if(!badge)return;if(!senimeSignedV247()){badge.classList.add('hidden');return}try{const rows=await senimeFetchV247(`notifications?user_id=eq.${senimeMeV247()}&read_at=is.null&select=id&limit=99`),n=rows?.length||0;badge.textContent=n>99?'99+':String(n);badge.classList.toggle('hidden',!n);const c=$('#communityNotificationCountV247');if(c)c.textContent=n?String(n):''}catch{badge.classList.add('hidden')}}
async function loadNotificationsV247(render=true){if(!senimeSignedV247())return;try{const rows=await senimeFetchV247(`notifications?user_id=eq.${senimeMeV247()}&select=id,user_id,actor_id,kind,title,body,payload,created_at,read_at&order=created_at.desc&limit=100`);communityV247.notifications=rows||[];const profs=await publicProfilesByIdsV246((rows||[]).map(x=>x.actor_id));for(const p of profs||[])socialStateV241.profiles.set(p.id,p);if(render)renderNotificationsV247();loadNotificationBadgeV247()}catch(e){if(render){const b=$('#communityNotificationsV247');if(b)b.innerHTML='<div class="community-empty-v247">Уведомления пока недоступны.<br><small>Выполни SUPABASE_PATCH_V247.sql.</small></div>'}}}
function renderNotificationsV247(){const box=$('#communityNotificationsV247');if(!box)return;const unread=communityV247.notifications.filter(n=>!n.read_at).length;box.innerHTML=`<div class="notifications-page-v247"><header class="community-page-head-v247"><div><span>INBOX</span><h2>Уведомления</h2><p>Лайки, ответы, подписки, заявки и сообщения.</p></div><button onclick="markAllNotificationsV247()" class="secondary">✓ Прочитать всё</button></header><div class="notification-list-v247">${communityV247.notifications.length?communityV247.notifications.map(n=>{const a=socialStateV241.profiles.get(n.actor_id)||{};return `<button class="notification-card-v247 ${n.read_at?'':'unread'}" onclick="openNotificationV247(${Number(n.id)})"><span class="notification-icon-v247">${notificationIconV247(n.kind)}</span><span class="notification-actor-v247">${socialAvatarV241(a)}</span><span class="notification-copy-v247"><b>${esc(n.title)}</b><small>${esc(n.body||'')}</small><time>${publicCommentTimeV246(n.created_at)}</time></span><i>›</i></button>`}).join(''):'<div class="community-empty-v247">Пока тихо.</div>'}</div></div>`;const c=$('#communityNotificationCountV247');if(c)c.textContent=unread?String(unread):''}
async function markNotificationReadV247(id){try{await senimeFetchV247(`notifications?id=eq.${id}`,{method:'PATCH',body:{read_at:new Date().toISOString()},prefer:'return=minimal'})}catch{}}
async function markAllNotificationsV247(){try{await senimeFetchV247(`notifications?user_id=eq.${senimeMeV247()}&read_at=is.null`,{method:'PATCH',body:{read_at:new Date().toISOString()},prefer:'return=minimal'});await loadNotificationsV247(true)}catch(e){profileToastV16('Не удалось',e.message||String(e))}}
async function focusDeepV247(sel){for(let i=0;i<22;i++){const e=document.querySelector(sel);if(e){e.scrollIntoView({behavior:'smooth',block:'center'});e.classList.add('deep-highlight-v247');setTimeout(()=>e.classList.remove('deep-highlight-v247'),2200);return true}await senimeDelayV247(120)}return false}
async function openProfileCommentDeepV247(p){const pid=p?.profile_id,cid=p?.comment_id||p?.root_id;if(!pid)return;closeCommunityV247();if(String(pid)===String(senimeMeV247())){openProfileV16();setProfileTabV16('overview');await senimeDelayV247(180);await loadOwnGuestbookV2461?.();await focusDeepV247(`#profile-comment-${cid}`)}else{await socialOpenPublicProfileV242(pid);await focusDeepV247(`#profile-comment-${cid}`)}}
async function openAnimeCommentDeepV247(p){closeCommunityV247();const id=Number(p?.anime_id)||0;if(!id)return;try{await openSharedAnimeV247(id);await senimeDelayV247(160);if(openedDetail)openWatchPlayerV15(openedDetail.section,openedDetail.index,{season:Math.max(0,(Number(p.season)||1)-1),episode:Math.max(1,Number(p.episode)||1)});await senimeDelayV247(300);watchCommentScopeV151=p.scope||'title';await loadAnimeCommentsV247(true);await focusDeepV247(`#anime-comment-${p.comment_id||p.root_id}`)}catch(e){profileToastV16('Не удалось открыть комментарий',e.message||String(e))}}
async function openNotificationV247(id){const n=communityV247.notifications.find(x=>Number(x.id)===Number(id));if(!n)return;await markNotificationReadV247(id);const p=n.payload||{};if(['profile_comment_like','profile_comment_reply','profile_comment'].includes(n.kind))await openProfileCommentDeepV247(p);else if(['anime_comment_like','anime_comment_reply'].includes(n.kind))await openAnimeCommentDeepV247(p);else if(n.kind==='dm'||n.kind==='message_reaction'){communityV247.selectedChat=p.user_id||n.actor_id;await setCommunityTabV247('chats')}else if(n.kind==='friend_request')await setCommunityTabV247('friends');else if(n.kind==='follow')await openUserProfileV247(p.user_id||n.actor_id);else if(n.kind==='collection_like')await openAnimeCollectionV247(p.collection_id);await loadNotificationBadgeV247()}
Object.assign(window,{openNotificationV247,markAllNotificationsV247});setInterval(loadNotificationBadgeV247,20000);setTimeout(loadNotificationBadgeV247,1000);

/* Leaderboards */
const LEADER_METRICS_V247=[['level','🏆 Уровень'],['hours','⏱ Часы просмотра'],['episodes','▶ Серии'],['titles','🎬 Тайтлы'],['collection','🃏 Коллекция']];
function leaderboardValueV247(metric,v){const n=Number(v)||0;if(metric==='hours')return `${n.toFixed(n<10?1:0)} ч`;if(metric==='level')return `LVL ${Math.round(n)}`;if(metric==='collection')return `${Math.round(n)} карт`;if(metric==='episodes')return `${Math.round(n)} серий`;return `${Math.round(n)} тайтлов`}
async function loadLeaderboardV247(metric='level'){communityV247.leaderMetric=metric;const box=$('#communityLeaderboardsV247');if(!box)return;box.innerHTML='<div class="community-loading-v247">Собираю рейтинг…</div>';try{if(senimeSignedV247())socialSyncPublicProfileV242?.().catch(()=>{});const top=await senimeRpcV247('senime_leaderboard',{metric,limit_rows:50},senimeSignedV247())||[],me=senimeMeV247(),my=top.findIndex(x=>x.id===me);box.innerHTML=`<div class="leaderboards-page-v247"><header class="community-page-head-v247"><div><span>RANKED</span><h2>Лидерборды Senime</h2><p>Premium выделяется визуально, но не даёт бонус к месту.</p></div>${my>=0?`<div class="my-rank-v247">Твоё место <b>#${my+1}</b></div>`:''}</header><div class="leader-metrics-v247">${LEADER_METRICS_V247.map(x=>`<button class="${metric===x[0]?'active':''}" onclick="loadLeaderboardV247('${x[0]}')">${x[1]}</button>`).join('')}</div>${top.length>=3?`<div class="leader-podium-v247">${[1,0,2].map(idx=>{const r=top[idx];if(!r)return'';const pp=r.public_payload||{},premium=Number(pp.premiumUntil||0)>Date.now();return `<button class="leader-podium-card-v247 place-${idx+1}" onclick="openUserProfileV247('${r.id}')"><em>#${idx+1}</em><span>${socialAvatarV241(r)}</span><b>${esc(r.display_name||r.handle||'Пользователь')}</b>${socialFounderBadgeV242(r)}${premium?'<i>👑 PREMIUM</i>':''}<strong>${leaderboardValueV247(metric,r.value)}</strong></button>`}).join('')}</div>`:''}<div class="leader-table-v247">${top.map((r,i)=>{const premium=Number(r.public_payload?.premiumUntil||0)>Date.now();return `<button class="${r.id===me?'me':''}" onclick="openUserProfileV247('${r.id}')"><em>#${i+1}</em><span class="leader-avatar-v247">${socialAvatarV241(r)}</span><span><b>${esc(r.display_name||r.handle||'Пользователь')} ${socialFounderBadgeV242(r)}</b><small>${esc(r.handle?'@'+r.handle:'')}${premium?' · 👑 Premium':''}</small></span><strong>${leaderboardValueV247(metric,r.value)}</strong></button>`}).join('')||'<div class="community-empty-v247">Пока недостаточно данных.</div>'}</div></div>`}catch(e){box.innerHTML='<div class="community-empty-v247">Лидерборды пока недоступны.<br><small>Выполни SUPABASE_PATCH_V247.sql.</small></div>'}}
window.loadLeaderboardV247=loadLeaderboardV247;

/* User-made anime collections */
const collectionHubV247={public:[],mine:[],open:null};
function collectionCardV247(c,mine=false){return `<button class="anime-curation-card-v247" onclick="openAnimeCollectionV247('${c.id}')">${c.cover_url?`<img src="${esc(c.cover_url)}" alt="">`:'<div class="curation-cover-empty-v247">📚</div>'}<span><small>${mine?'МОЯ ПОДБОРКА':'ОТ ПОЛЬЗОВАТЕЛЯ'}</small><b>${esc(c.title)}</b><p>${esc(c.description||'')}</p></span></button>`}
async function loadCollectionsHubV247(){const box=$('#communityCollectionsV247');if(!box)return;box.innerHTML='<div class="community-loading-v247">Загружаю подборки…</div>';try{collectionHubV247.public=await senimeFetchV247('anime_collections?visibility=eq.public&select=id,owner_id,title,description,visibility,cover_url,created_at,updated_at&order=updated_at.desc&limit=40',{auth:false})||[];collectionHubV247.mine=senimeSignedV247()?await senimeFetchV247(`anime_collections?owner_id=eq.${senimeMeV247()}&select=id,owner_id,title,description,visibility,cover_url,created_at,updated_at&order=updated_at.desc&limit=50`)||[]:[];renderCollectionsHubV247()}catch(e){box.innerHTML='<div class="community-empty-v247">Подборки пока недоступны.<br><small>Выполни SUPABASE_PATCH_V247.sql.</small></div>'}}
function renderCollectionsHubV247(){const box=$('#communityCollectionsV247');if(!box)return;box.innerHTML=`<div class="collections-hub-v247"><header class="community-page-head-v247"><div><span>CURATIONS</span><h2>Подборки аниме</h2><p>Собирай свои списки и находи рекомендации других пользователей.</p></div>${senimeSignedV247()?'<button class="primary" onclick="openCreateCollectionV247()">＋ Создать подборку</button>':'<button onclick="openAccountV23()">Войти</button>'}</header>${collectionHubV247.mine.length?`<section class="community-section-v247"><h3>Мои подборки <i>${collectionHubV247.mine.length}</i></h3><div class="curation-grid-v247">${collectionHubV247.mine.map(c=>collectionCardV247(c,true)).join('')}</div></section>`:''}<section class="community-section-v247"><h3>Новые от пользователей <i>${collectionHubV247.public.length}</i></h3><div class="curation-grid-v247">${collectionHubV247.public.length?collectionHubV247.public.map(c=>collectionCardV247(c)).join(''):'<div class="community-empty-v247">Пока никто ничего не собрал.</div>'}</div></section></div>`}
function ensureCollectionEditorV247(){if($('#collectionEditorV247'))return;document.body.insertAdjacentHTML('beforeend',`<div id="collectionEditorV247" class="mini-overlay-v247 hidden"><div class="mini-card-v247"><button class="mini-close-v247" onclick="closeCollectionEditorV247()">✕</button><span>NEW COLLECTION</span><h3>Новая подборка</h3><label>Название<input id="collectionTitleV247" maxlength="80" placeholder="Аниме, после которых смотришь в потолок"></label><label>Описание<textarea id="collectionDescV247" maxlength="500" rows="4"></textarea></label><label>Видимость<select id="collectionVisibilityV247"><option value="public">Публичная</option><option value="unlisted">Только по ссылке</option><option value="private">Личная</option></select></label><button id="collectionCreateSaveV247" class="primary">Создать</button></div></div>`);$('#collectionCreateSaveV247').onclick=createCollectionV247}
function openCreateCollectionV247(){if(!senimeSignedV247()){openAccountV23();return}ensureCollectionEditorV247();$('#collectionEditorV247').classList.remove('hidden')}
function closeCollectionEditorV247(){$('#collectionEditorV247')?.classList.add('hidden')}
async function createCollectionV247(){const title=String($('#collectionTitleV247')?.value||'').trim();if(!title)return;try{const id=await senimeRpcV247('senime_create_collection',{c_title:title,c_description:String($('#collectionDescV247')?.value||'').trim(),c_visibility:$('#collectionVisibilityV247')?.value||'public'});closeCollectionEditorV247();profileToastV16('📚 Подборка создана',title);await loadCollectionsHubV247();if(id)openAnimeCollectionV247(String(id).replace(/"/g,''))}catch(e){profileToastV16('Не удалось создать',e.message||String(e))}}
Object.assign(window,{openCreateCollectionV247,closeCollectionEditorV247});

async function openAnimeCollectionV247(id){ensureCommunityModalV247();$('#communityModalV247').classList.remove('hidden');document.body.style.overflow='hidden';document.querySelectorAll('.community-pane-v247').forEach(p=>p.classList.remove('active'));$('#communityCollectionsV247')?.classList.add('active');document.querySelectorAll('#communityTabsV247 [data-community-tab]').forEach(b=>b.classList.toggle('active',b.dataset.communityTab==='collections'));const box=$('#communityCollectionsV247');if(box)box.innerHTML='<div class="community-loading-v247">Открываю подборку…</div>';try{const c=(await senimeFetchV247(`anime_collections?id=eq.${id}&select=id,owner_id,title,description,visibility,cover_url,created_at,updated_at&limit=1`,{auth:senimeSignedV247()}))?.[0];if(!c)throw new Error('Подборка не найдена');const items=await senimeFetchV247(`anime_collection_items?collection_id=eq.${id}&select=collection_id,anime_id,anime_title,cover_url,note,sort_order,added_at&order=sort_order.asc`,{auth:senimeSignedV247()}),likes=await senimeFetchV247(`anime_collection_likes?collection_id=eq.${id}&select=user_id`,{auth:false}),owner=(await publicProfilesByIdsV246([c.owner_id]))?.[0]||{},mine=c.owner_id===senimeMeV247(),liked=(likes||[]).some(x=>x.user_id===senimeMeV247());let saved=false;if(senimeSignedV247())saved=!!(await senimeFetchV247(`anime_collection_saves?collection_id=eq.${id}&user_id=eq.${senimeMeV247()}&select=collection_id`))?.length;collectionHubV247.open=c;box.innerHTML=`<div class="curation-detail-v247"><button class="curation-back-v247" onclick="loadCollectionsHubV247()">‹ Все подборки</button><header><div class="curation-owner-v247"><button onclick="openUserProfileV247('${c.owner_id}')">${socialAvatarV241(owner)}</button><span><small>ПОДБОРКА ОТ</small><b onclick="openUserProfileV247('${c.owner_id}')">${esc(owner.display_name||owner.handle||'Пользователь')} ${socialFounderBadgeV242(owner)}</b></span></div><div class="curation-actions-v247">${senimeSignedV247()?`<button class="${liked?'active':''}" onclick="toggleCollectionLikeV247('${id}')">♥ ${likes?.length||0}</button><button class="${saved?'active':''}" onclick="toggleCollectionSaveV247('${id}')">${saved?'✓ Сохранено':'☆ Сохранить'}</button>`:''}${mine?`<button class="danger" onclick="deleteCollectionV247('${id}')">Удалить</button>`:''}</div></header><div class="curation-hero-v247">${c.cover_url?`<img src="${esc(c.cover_url)}" alt="">`:''}<div><span>${esc(c.visibility.toUpperCase())}</span><h2>${esc(c.title)}</h2><p>${esc(c.description||'Без описания')}</p><small>${items?.length||0} аниме · ${likes?.length||0} ♥</small></div></div><div class="curation-items-v247">${items?.length?items.map((x,i)=>`<article><button class="curation-anime-open-v247" onclick="openSharedAnimeV247(${Number(x.anime_id)})">${x.cover_url?`<img src="${esc(x.cover_url)}" alt="">`:'<i>🎌</i>'}<span><small>#${i+1}</small><b>${esc(x.anime_title)}</b>${x.note?`<p>${esc(x.note)}</p>`:''}</span></button>${mine?`<div class="curation-owner-tools-v247"><button onclick="moveCollectionItemV247('${id}',${Number(x.anime_id)},-1)">↑</button><button onclick="moveCollectionItemV247('${id}',${Number(x.anime_id)},1)">↓</button><button class="danger" onclick="removeCollectionItemV247('${id}',${Number(x.anime_id)})">✕</button></div>`:''}</article>`).join(''):'<div class="community-empty-v247">Здесь пока нет аниме.</div>'}</div></div>`}catch(e){if(box)box.innerHTML=`<div class="community-empty-v247">${esc(e.message||String(e))}</div>`}}
async function toggleCollectionLikeV247(id){try{await senimeRpcV247('senime_toggle_collection_like',{cid:id});await openAnimeCollectionV247(id)}catch(e){profileToastV16('Не удалось',e.message||String(e))}}
async function toggleCollectionSaveV247(id){try{await senimeRpcV247('senime_toggle_collection_save',{cid:id});await openAnimeCollectionV247(id)}catch(e){profileToastV16('Не удалось',e.message||String(e))}}
async function deleteCollectionV247(id){if(!confirm('Удалить подборку?'))return;try{await senimeRpcV247('senime_delete_collection',{cid:id});await loadCollectionsHubV247()}catch(e){profileToastV16('Не удалось удалить',e.message||String(e))}}
async function removeCollectionItemV247(id,a){try{await senimeRpcV247('senime_remove_collection_item',{cid:id,a_id:a});await openAnimeCollectionV247(id)}catch(e){profileToastV16('Не удалось',e.message||String(e))}}
async function moveCollectionItemV247(id,a,direction){try{await senimeRpcV247('senime_move_collection_item',{cid:id,a_id:a,direction});await openAnimeCollectionV247(id)}catch(e){profileToastV16('Не удалось',e.message||String(e))}}
Object.assign(window,{openAnimeCollectionV247,loadCollectionsHubV247,toggleCollectionLikeV247,toggleCollectionSaveV247,deleteCollectionV247,removeCollectionItemV247,moveCollectionItemV247});

function openedAnimeInfoV247(){if(!openedDetail)return null;const e=latestData.sections?.[openedDetail.section]?.[openedDetail.index];return e?{id:Number(e.anilist_id)||0,title:e.title||'',cover:e.cover||''}:null}
function ensureCollectionPickerV247(){if($('#collectionPickerV247'))return;document.body.insertAdjacentHTML('beforeend',`<div id="collectionPickerV247" class="mini-overlay-v247 hidden"><div class="mini-card-v247 collection-picker-card-v247"><button class="mini-close-v247" onclick="closeCollectionPickerV247()">✕</button><span>ADD TO COLLECTION</span><h3>Добавить в подборку</h3><div id="collectionPickerListV247"></div><button class="secondary" onclick="closeCollectionPickerV247();openCommunityV247('collections');setTimeout(openCreateCollectionV247,100)">＋ Новая подборка</button></div></div>`)}
async function addOpenedAnimeToCollectionV247(){if(!senimeSignedV247()){openAccountV23();return}const a=openedAnimeInfoV247();if(!a?.id){profileToastV16('Нет AniList ID');return}ensureCollectionPickerV247();$('#collectionPickerV247').classList.remove('hidden');const list=$('#collectionPickerListV247');try{const rows=await senimeFetchV247(`anime_collections?owner_id=eq.${senimeMeV247()}&select=id,title,visibility,cover_url&order=updated_at.desc`);list.innerHTML=rows?.length?rows.map(c=>`<button onclick="addOpenedAnimeToCollectionIdV247('${c.id}')"><span>${c.cover_url?`<img src="${esc(c.cover_url)}" alt="">`:'📚'}</span><b>${esc(c.title)}<small>${esc(c.visibility)}</small></b><i>＋</i></button>`).join(''):'<div class="community-empty-v247">Сначала создай подборку.</div>'}catch(e){list.innerHTML=`<div class="community-empty-v247">${esc(e.message||String(e))}</div>`}}
function closeCollectionPickerV247(){$('#collectionPickerV247')?.classList.add('hidden')}
async function addOpenedAnimeToCollectionIdV247(id){const a=openedAnimeInfoV247();if(!a)return;try{await senimeRpcV247('senime_add_collection_item',{cid:id,a_id:a.id,a_title:a.title,a_cover:a.cover,a_note:''});closeCollectionPickerV247();profileToastV16('📚 Добавлено',a.title)}catch(e){profileToastV16('Не удалось',e.message||String(e))}}
Object.assign(window,{addOpenedAnimeToCollectionV247,closeCollectionPickerV247,addOpenedAnimeToCollectionIdV247});$('#detailAddCollectionV247')?.addEventListener('click',addOpenedAnimeToCollectionV247);

async function appendPublicCollectionsV247(id){const canvas=document.querySelector('#publicProfileBodyV242 .public-profile-canvas-v243');if(!canvas)return;canvas.querySelector('.public-curations-v247')?.remove();try{const rows=await senimeFetchV247(`anime_collections?owner_id=eq.${id}&visibility=eq.public&select=id,title,description,cover_url&order=updated_at.desc&limit=4`,{auth:false});if(!rows?.length)return;const s=document.createElement('section');s.className='public-widget-v243 public-curations-v247';s.innerHTML=`<div class="public-widget-head-v243"><div><span>ПОДБОРКИ</span><h3>Подборки пользователя</h3></div><small>${rows.length}</small></div><div class="public-curations-grid-v247">${rows.map(c=>`<button onclick="openAnimeCollectionV247('${c.id}')">${c.cover_url?`<img src="${esc(c.cover_url)}" alt="">`:'<i>📚</i>'}<b>${esc(c.title)}</b><small>${esc(c.description||'')}</small></button>`).join('')}</div>`;canvas.appendChild(s)}catch{}}
const socialOpenPublicProfileBeforeCollectionsV247=socialOpenPublicProfileV242;
socialOpenPublicProfileV242=async function(id){await socialOpenPublicProfileBeforeCollectionsV247(id);await appendPublicCollectionsV247(id)};window.socialOpenPublicProfileV242=socialOpenPublicProfileV242;

/* Share button now shares a real deep link to the anime. */
async function shareOpenedAnimeV247(){const a=openedAnimeInfoV247();if(!a?.id)return;const u=new URL(location.href);u.searchParams.set('anime',String(a.id));u.hash='';try{if(navigator.share&&location.protocol!=='file:')await navigator.share({title:`${a.title} — Senime`,text:`Посмотри ${a.title} в Senime`,url:u.toString()});else{await copyText(u.toString());profileToastV16('Ссылка скопирована',a.title)}dailyRecordV166?.('shares',1)}catch(e){if(e?.name!=='AbortError')profileToastV16('Не удалось поделиться',e.message||String(e))}}
shareOpenedAnimeV166=shareOpenedAnimeV247;
async function openSharedAnimeV247(id){id=Number(id)||0;if(!id)return;try{const m=await fetchBundleNodeV9(id);if(!m)throw new Error('Аниме не найдено');catalogStateV10.media.set(id,m);await openCatalogMediaV10(id)}catch(e){profileToastV16('Не удалось открыть аниме',e.message||String(e))}}
window.openSharedAnimeV247=openSharedAnimeV247;setTimeout(()=>{const id=Number(new URL(location.href).searchParams.get('anime'))||0;if(id)openSharedAnimeV247(id)},1400);

/* Profile comment likes and clickable authors. */
const profileCommentLikesV247=new Map();
async function fetchProfileCommentLikesV247(comments){profileCommentLikesV247.clear();const ids=(comments||[]).map(x=>Number(x.id)).filter(Boolean);if(!ids.length)return;try{const rows=await publicRestV246(`profile_comment_likes?comment_id=in.(${ids.join(',')})&select=comment_id,user_id`);for(const r of rows||[]){const k=String(r.comment_id),x=profileCommentLikesV247.get(k)||{count:0,mine:false};x.count++;if(r.user_id===senimeMeV247())x.mine=true;profileCommentLikesV247.set(k,x)}}catch{}}
function profileLikeStateV247(id){return profileCommentLikesV247.get(String(id))||{count:0,mine:false}}
async function toggleProfileCommentLikeCloudV247(id,source='public'){if(!senimeSignedV247()){openAccountV23();return}try{await senimeRpcV247('senime_toggle_profile_comment_like',{comment_id:Number(id)});dailyRecordV166?.('likes',1);if(source==='own')await loadOwnGuestbookV2461();else await renderPublicProfileCommentsV246(publicProfileActiveV246)}catch(e){profileToastV16('Лайк не поставился',e.message||String(e))}}
window.toggleProfileCommentLikeCloudV247=toggleProfileCommentLikeCloudV247;
const fetchPublicCommentsBeforeLikesV247=fetchPublicCommentsV246;
fetchPublicCommentsV246=async function(profileId){const rows=await fetchPublicCommentsBeforeLikesV247(profileId);await fetchProfileCommentLikesV247(publicProfileCommentsV246);return rows};
publicCommentHtmlV246=function(c,replies=[]){const a=publicProfileAuthorsV246.get(c.author_id)||{},creator=socialFounderBadgeV242?.(a)||'',like=profileLikeStateV247(c.id);return `<article id="profile-comment-${c.id}" class="public-comment-v246"><button class="public-comment-avatar-v246 social-comment-author-v247" onclick="openUserProfileV247('${c.author_id}')">${publicCommentAvatarV246(a)}</button><div class="public-comment-copy-v246"><div class="public-comment-meta-v246"><button class="social-comment-name-v247" onclick="openUserProfileV247('${c.author_id}')">${esc(a.display_name||a.handle||'Пользователь')} ${creator}</button><time>${publicCommentTimeV246(c.created_at)}</time></div><p>${esc(c.body||'')}</p><div class="public-comment-actions-v246"><button class="${like.mine?'liked':''}" onclick="toggleProfileCommentLikeCloudV247(${Number(c.id)},'public')">♥ <span>${like.count}</span></button>${accountSignedInV236?.()?`<button onclick="publicStartReplyV246(${Number(c.id)},'${attrV247(a.display_name||a.handle||'Пользователь')}')">↩ Ответить</button>`:''}${publicCommentCanDeleteV246(c)?`<button class="danger" onclick="publicDeleteCommentV246(${Number(c.id)})">Удалить</button>`:''}</div>${replies.length?`<div class="public-comment-replies-v246">${replies.map(publicReplyHtmlV246).join('')}</div>`:''}</div></article>`};
publicReplyHtmlV246=function(r){const a=publicProfileAuthorsV246.get(r.author_id)||{},creator=socialFounderBadgeV242?.(a)||'',like=profileLikeStateV247(r.id);return `<div id="profile-comment-${r.id}" class="public-comment-reply-v246"><button class="public-comment-avatar-v246 small social-comment-author-v247" onclick="openUserProfileV247('${r.author_id}')">${publicCommentAvatarV246(a)}</button><div><div class="public-comment-meta-v246"><button class="social-comment-name-v247" onclick="openUserProfileV247('${r.author_id}')">${esc(a.display_name||a.handle||'Пользователь')} ${creator}</button><time>${publicCommentTimeV246(r.created_at)}</time></div><p>${esc(r.body||'')}</p><div class="public-comment-actions-v246"><button class="${like.mine?'liked':''}" onclick="toggleProfileCommentLikeCloudV247(${Number(r.id)},'public')">♥ <span>${like.count}</span></button>${accountSignedInV236?.()?`<button onclick="publicStartReplyV246(${Number(r.parent_id||r.id)},'${attrV247(a.display_name||a.handle||'Пользователь')}')">↩ Ответить</button>`:''}${publicCommentCanDeleteV246(r)?`<button class="danger" onclick="publicDeleteCommentV246(${Number(r.id)})">Удалить</button>`:''}</div></div></div>`};

/* Own profile guestbook uses the same likes and deep IDs. */
const loadOwnGuestbookBeforeLikesV247=loadOwnGuestbookV2461;
loadOwnGuestbookV2461=async function(){await loadOwnGuestbookBeforeLikesV247();await fetchProfileCommentLikesV247(ownGuestCommentsV2461);renderOwnGuestCachedV2461()};
function ownGuestReplyHtmlV247(r){const a=ownGuestAuthorsV2461.get(r.author_id)||{},like=profileLikeStateV247(r.id);return `<div id="profile-comment-${r.id}" class="profile-comment-reply cloud-v2461"><button class="profile-comment-avatar small social-comment-author-v247" onclick="openUserProfileV247('${r.author_id}')">${ownGuestAuthorHtmlV2461(a)}</button><div class="profile-comment-copy"><div class="profile-comment-meta"><button class="social-comment-name-v247" onclick="openUserProfileV247('${r.author_id}')">${esc(a.display_name||a.handle||'Пользователь')} ${socialFounderBadgeV242(a)||''}</button><time>${publicCommentTimeV246(r.created_at)}</time></div><p>${esc(r.body||'')}</p><div class="comment-actions"><button class="${like.mine?'liked':''}" onclick="toggleProfileCommentLikeCloudV247(${Number(r.id)},'own')">♥ <span>${like.count}</span></button><button onclick="ownGuestStartReplyV2461(${Number(r.parent_id||r.id)},'${attrV247(a.display_name||a.handle||'Пользователь')}')">↩ Ответить</button>${ownGuestCanDeleteV2461(r)?`<button class="danger" onclick="ownGuestDeleteV2461(${Number(r.id)})">Удалить</button>`:''}</div></div></div>`}
function ownGuestRootHtmlV247(c,replies=[]){const a=ownGuestAuthorsV2461.get(c.author_id)||{},like=profileLikeStateV247(c.id);return `<div id="profile-comment-${c.id}" class="profile-comment-thread cloud-v2461"><article class="profile-comment-card"><button class="profile-comment-avatar social-comment-author-v247" onclick="openUserProfileV247('${c.author_id}')">${ownGuestAuthorHtmlV2461(a)}</button><div class="profile-comment-copy"><div class="profile-comment-meta"><button class="social-comment-name-v247" onclick="openUserProfileV247('${c.author_id}')">${esc(a.display_name||a.handle||'Пользователь')} ${socialFounderBadgeV242(a)||''}</button><time>${publicCommentTimeV246(c.created_at)}</time></div><p>${esc(c.body||'')}</p><div class="comment-actions"><button class="${like.mine?'liked':''}" onclick="toggleProfileCommentLikeCloudV247(${Number(c.id)},'own')">♥ <span>${like.count}</span></button><button onclick="ownGuestStartReplyV2461(${Number(c.id)},'${attrV247(a.display_name||a.handle||'Пользователь')}')">↩ Ответить</button><span>${replies.length} ответов</span>${ownGuestCanDeleteV2461(c)?`<button class="danger" onclick="ownGuestDeleteV2461(${Number(c.id)})">Удалить</button>`:''}</div></div></article>${replies.map(ownGuestReplyHtmlV247).join('')}</div>`}
renderOwnGuestCachedV2461=function(){const list=$('#profileCommentsList'),count=$('#profileCommentsCount');if(!list)return;const roots=ownGuestCommentsV2461.filter(x=>!x.parent_id),by=new Map();for(const r of ownGuestCommentsV2461.filter(x=>x.parent_id)){const k=String(r.parent_id);if(!by.has(k))by.set(k,[]);by.get(k).push(r)}if(count)count.textContent=`${ownGuestCommentsV2461.length} записей`;list.innerHTML=roots.length?roots.map(c=>ownGuestRootHtmlV247(c,by.get(String(c.id))||[])).join(''):'<div class="profile-comment-empty"><b>Пока тихо</b><span>Комментарии других пользователей появятся здесь.</span></div>';const input=$('#profileCommentInput');if(input)input.placeholder=ownGuestReplyV2461.parentId?`Ответ для ${ownGuestReplyV2461.name}…`:'Оставить запись в своей гостевой…'};

/* Player/anime comments move from localStorage to Senime Cloud. */
const animeCommentV247={key:'',items:[],authors:new Map(),likes:new Map(),replyTo:null,sort:'new',loading:false};
function animeCommentContextV247(){const e=watchStateV15.entry;if(!e)return null;const id=Number(e.anilist_id)||0;if(!id)return null;const scope=watchCommentScopeV151||'episode',season=scope==='title'?0:watchStateV15.season+1,episode=scope==='episode'?watchStateV15.episode:0;return {id,title:e.title||'',scope,season,episode,key:`${id}:${scope}:${season}:${episode}`}}
function animeLikeV247(id){return animeCommentV247.likes.get(String(id))||{count:0,mine:false}}
function animeCommentCanDeleteV247(c){return !!senimeMeV247()&&(c.author_id===senimeMeV247()||founderSessionV245?.())}
async function loadAnimeCommentsV247(force=false){const ctx=animeCommentContextV247();if(!ctx)return;if(animeCommentV247.loading)return;if(!force&&animeCommentV247.key===ctx.key&&animeCommentV247.items.length){renderAnimeCommentsCachedV247();return}animeCommentV247.loading=true;animeCommentV247.key=ctx.key;const list=$('#watchCommentsList');if(list)list.innerHTML='<div class="watch-comments-empty">Загружаю обсуждение Senime…</div>';try{const q=new URLSearchParams();q.set('anime_id',`eq.${ctx.id}`);q.set('scope',`eq.${ctx.scope}`);q.set('season',`eq.${ctx.season}`);q.set('episode',`eq.${ctx.episode}`);q.set('select','id,anime_id,anime_title,scope,season,episode,author_id,parent_id,body,created_at');q.set('order','created_at.asc');q.set('limit','300');animeCommentV247.items=await publicRestV246(`anime_comments?${q.toString()}`)||[];const profs=await publicProfilesByIdsV246([...new Set(animeCommentV247.items.map(x=>x.author_id))]);animeCommentV247.authors=new Map((profs||[]).map(x=>[x.id,x]));animeCommentV247.likes=new Map();const ids=animeCommentV247.items.map(x=>x.id);if(ids.length){const likes=await publicRestV246(`anime_comment_likes?comment_id=in.(${ids.join(',')})&select=comment_id,user_id`);for(const r of likes||[]){const k=String(r.comment_id),x=animeCommentV247.likes.get(k)||{count:0,mine:false};x.count++;if(r.user_id===senimeMeV247())x.mine=true;animeCommentV247.likes.set(k,x)}}renderAnimeCommentsCachedV247()}catch(e){if(list)list.innerHTML='<div class="watch-comments-empty">Обсуждение пока недоступно.<br><small>Выполни SUPABASE_PATCH_V247.sql.</small></div>'}finally{animeCommentV247.loading=false}}
function animeReplyNodeV247(r){const a=animeCommentV247.authors.get(r.author_id)||{},like=animeLikeV247(r.id);return `<div id="anime-comment-${r.id}" class="watch-comment-reply"><button class="watch-comment-avatar small social-comment-author-v247" onclick="openUserProfileV247('${r.author_id}')">${socialAvatarV241(a)}</button><div class="watch-comment-main"><div><button class="social-comment-name-v247" onclick="openUserProfileV247('${r.author_id}')">${esc(a.display_name||a.handle||'Пользователь')} ${socialFounderBadgeV242(a)||''}</button><small>${publicCommentTimeV246(r.created_at)}</small></div><p>${esc(r.body||'')}</p><div class="comment-actions"><button class="${like.mine?'liked':''}" onclick="toggleAnimeCommentLikeV247(${Number(r.id)})">♥ <span>${like.count}</span></button>${senimeSignedV247()?`<button onclick="startAnimeCommentReplyV247(${Number(r.parent_id||r.id)},'${attrV247(a.display_name||a.handle||'Пользователь')}')">↩ Ответить</button>`:''}${animeCommentCanDeleteV247(r)?`<button class="danger" onclick="deleteAnimeCommentV247(${Number(r.id)})">Удалить</button>`:''}</div></div></div>`}
function animeCommentNodeV247(c,replies=[]){const a=animeCommentV247.authors.get(c.author_id)||{},like=animeLikeV247(c.id);return `<article id="anime-comment-${c.id}" class="watch-comment-card"><button class="watch-comment-avatar social-comment-author-v247" onclick="openUserProfileV247('${c.author_id}')">${socialAvatarV241(a)}</button><div class="watch-comment-main"><div><button class="social-comment-name-v247" onclick="openUserProfileV247('${c.author_id}')">${esc(a.display_name||a.handle||'Пользователь')} ${socialFounderBadgeV242(a)||''}</button><small>${publicCommentTimeV246(c.created_at)}</small></div><p>${esc(c.body||'')}</p><div class="comment-actions"><button class="${like.mine?'liked':''}" onclick="toggleAnimeCommentLikeV247(${Number(c.id)})">♥ <span>${like.count}</span></button>${senimeSignedV247()?`<button onclick="startAnimeCommentReplyV247(${Number(c.id)},'${attrV247(a.display_name||a.handle||'Пользователь')}')">↩ Ответить</button>`:''}<span>${replies.length} ответов</span>${animeCommentCanDeleteV247(c)?`<button class="danger" onclick="deleteAnimeCommentV247(${Number(c.id)})">Удалить</button>`:''}</div>${replies.length?`<div class="watch-comment-replies-v247">${replies.map(animeReplyNodeV247).join('')}</div>`:''}</div></article>`}
function renderAnimeCommentsCachedV247(){const list=$('#watchCommentsList'),ctx=$('#watchCommentContext');if(!list)return;if(ctx)ctx.textContent=commentContextTextV151();document.querySelectorAll('#watchCommentScope [data-scope]').forEach(b=>b.classList.toggle('active',b.dataset.scope===watchCommentScopeV151));let roots=animeCommentV247.items.filter(x=>!x.parent_id),by=new Map();for(const r of animeCommentV247.items.filter(x=>x.parent_id)){const k=String(r.parent_id);if(!by.has(k))by.set(k,[]);by.get(k).push(r)}if(animeCommentV247.sort==='popular')roots.sort((a,b)=>animeLikeV247(b.id).count-animeLikeV247(a.id).count||new Date(b.created_at)-new Date(a.created_at));else roots.sort((a,b)=>new Date(b.created_at)-new Date(a.created_at));const head=$('.watch-comments-head');if(head&&!$('#watchCommentSortV247')){const s=document.createElement('select');s.id='watchCommentSortV247';s.innerHTML='<option value="new">Новые</option><option value="popular">Популярные</option>';s.onchange=()=>{animeCommentV247.sort=s.value;renderAnimeCommentsCachedV247()};head.appendChild(s)}list.innerHTML=roots.length?roots.map(x=>animeCommentNodeV247(x,by.get(String(x.id))||[])).join(''):'<div class="watch-comments-empty">Пока здесь тихо. Напиши первый комментарий.</div>';if(animeCommentV247.replyTo)list.insertAdjacentHTML('beforeend',`<div class="watch-cloud-reply-v247"><span>↩ Ответ для <b>${esc(animeCommentV247.replyTo.name)}</b></span><button onclick="cancelAnimeCommentReplyV247()">✕</button></div>`)}
renderWatchCommentsV151=function(){loadAnimeCommentsV247(true)};
async function sendAnimeCommentV247(){if(!senimeSignedV247()){openAccountV23();return}const i=$('#watchCommentText'),body=String(i?.value||'').trim(),c=animeCommentContextV247();if(!body||!c)return;const b=$('#watchCommentSend');if(b)b.disabled=true;try{await senimeRpcV247('senime_add_anime_comment',{a_id:c.id,a_title:c.title,a_scope:c.scope,a_season:c.season,a_episode:c.episode,comment_body:body,reply_to:animeCommentV247.replyTo?.id||null});i.value='';animeCommentV247.replyTo=null;await loadAnimeCommentsV247(true)}catch(e){profileToastV16('Комментарий не отправлен',e.message||String(e))}finally{if(b)b.disabled=false}}
function startAnimeCommentReplyV247(id,name){animeCommentV247.replyTo={id:Number(id),name:String(name||'')};renderAnimeCommentsCachedV247();$('#watchCommentText')?.focus()}
function cancelAnimeCommentReplyV247(){animeCommentV247.replyTo=null;renderAnimeCommentsCachedV247()}
async function toggleAnimeCommentLikeV247(id){if(!senimeSignedV247()){openAccountV23();return}try{await senimeRpcV247('senime_toggle_anime_comment_like',{comment_id:Number(id)});dailyRecordV166?.('likes',1);await loadAnimeCommentsV247(true)}catch(e){profileToastV16('Лайк не поставился',e.message||String(e))}}
async function deleteAnimeCommentV247(id){try{await senimeRpcV247('senime_delete_anime_comment',{comment_id:Number(id)});await loadAnimeCommentsV247(true)}catch(e){profileToastV16('Не удалось удалить',e.message||String(e))}}
Object.assign(window,{sendAnimeCommentV247,startAnimeCommentReplyV247,cancelAnimeCommentReplyV247,toggleAnimeCommentLikeV247,deleteAnimeCommentV247});
function bindCloudWatchCommentsV247(){const oldT=$('#watchCommentText'),oldB=$('#watchCommentSend');if(!oldT||!oldB||oldB.dataset.cloudV247)return;const t=oldT.cloneNode(true),b=oldB.cloneNode(true);oldT.replaceWith(t);oldB.replaceWith(b);b.dataset.cloudV247='1';b.onclick=sendAnimeCommentV247;t.onkeydown=e=>{if((e.ctrlKey||e.metaKey)&&e.key==='Enter'){e.preventDefault();sendAnimeCommentV247()}};const scope=$('#watchCommentScope');if(scope&&!scope.dataset.cloudV247){scope.dataset.cloudV247='1';scope.addEventListener('click',()=>setTimeout(()=>{animeCommentV247.key='';loadAnimeCommentsV247(true)},50),true)}}
setTimeout(bindCloudWatchCommentsV247,300);const openWatchBeforeCommentsV247=openWatchPlayerV15;openWatchPlayerV15=function(...args){const r=openWatchBeforeCommentsV247(...args);setTimeout(()=>{animeCommentV247.key='';bindCloudWatchCommentsV247();loadAnimeCommentsV247(true)},300);return r};window.openWatchPlayerV15=openWatchPlayerV15;

/* Card collection organizer: search, filters, folders, hidden archive, compact mode. */
function ensureCollectionMetaV247(){const p=normalizeCollectionV166();p.cardFoldersV247=Array.isArray(p.cardFoldersV247)?p.cardFoldersV247:[];p.collectionUiV247=p.collectionUiV247||{query:'',rarity:'all',anime:'all',folder:'all',sort:'new',compact:false};for(const c of p.collection||[]){if(!Array.isArray(c.folderIds))c.folderIds=[];if(typeof c.hidden!=='boolean')c.hidden=false}return p}
function collectionUiV247(){return ensureCollectionMetaV247().collectionUiV247}
function cardRarityRankV247(r){return ({Legendary:4,Epic:3,Rare:2,Common:1})[String(r||'').replace(/^./,x=>x.toUpperCase())]||0}
function populateCollectionSelectsV247(){const p=ensureCollectionMetaV247(),ui=p.collectionUiV247,anime=$('#collectionAnimeV247'),folder=$('#collectionFolderV247');if(anime){const vals=[...new Set((p.collection||[]).map(c=>String(c.animeTitle||'')).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'ru'));anime.innerHTML='<option value="all">Все аниме</option>'+vals.map(x=>`<option value="${attrV247(x)}">${esc(x)}</option>`).join('');anime.value=vals.includes(ui.anime)?ui.anime:'all';ui.anime=anime.value}if(folder){folder.innerHTML='<option value="all">Все папки</option>'+p.cardFoldersV247.map(f=>`<option value="${f.id}">📁 ${esc(f.name)}</option>`).join('');folder.value=p.cardFoldersV247.some(f=>f.id===ui.folder)?ui.folder:'all';ui.folder=folder.value}}
function ensureCardFolderModalV247(){if($('#cardFolderModalV247'))return;document.body.insertAdjacentHTML('beforeend',`<div id="cardFolderModalV247" class="mini-overlay-v247 hidden"><div class="mini-card-v247"><button class="mini-close-v247" onclick="closeCardFolderModalV247()">✕</button><span>ORGANIZE</span><h3 id="cardFolderTitleV247">Папки карточек</h3><div id="cardFolderBodyV247"></div></div></div>`)}
function openCardFolderModalV247(characterId=null){ensureCardFolderModalV247();const p=ensureCollectionMetaV247(),c=characterId!=null?p.collection.find(x=>String(x.characterId)===String(characterId)):null,box=$('#cardFolderBodyV247');$('#cardFolderTitleV247').textContent=c?`Папки · ${c.name}`:'Папки карточек';box.innerHTML=`<div class="card-folder-create-v247"><input id="cardFolderNewNameV247" maxlength="40" placeholder="Новая папка"><button onclick="createCardFolderV247()">Создать</button></div>${p.cardFoldersV247.length?`<div class="card-folder-list-v247">${p.cardFoldersV247.map(f=>`<div><button class="${c?.folderIds.includes(f.id)?'active':''}" onclick="${c?`toggleCardFolderV247(${Number(c.characterId)},'${f.id}')`:`setCardFolderFilterV247('${f.id}')`}"><span>📁</span><b>${esc(f.name)}</b>${c?`<i>${c.folderIds.includes(f.id)?'✓':'＋'}</i>`:''}</button>${c?'':`<button class="danger" onclick="deleteCardFolderV247('${f.id}')">✕</button>`}</div>`).join('')}</div>`:'<div class="community-empty-v247">Создай первую папку.</div>'}`;$('#cardFolderModalV247').dataset.characterId=c?String(c.characterId):'';$('#cardFolderModalV247').classList.remove('hidden')}
function closeCardFolderModalV247(){$('#cardFolderModalV247')?.classList.add('hidden')}
function createCardFolderV247(){const p=ensureCollectionMetaV247(),name=String($('#cardFolderNewNameV247')?.value||'').trim();if(!name)return;const f={id:`f${Date.now().toString(36)}${Math.random().toString(36).slice(2,5)}`,name:name.slice(0,40)};p.cardFoldersV247.push(f);saveData();const cid=$('#cardFolderModalV247')?.dataset.characterId;openCardFolderModalV247(cid?Number(cid):null);renderCollectionV16()}
function toggleCardFolderV247(id,fid){const p=ensureCollectionMetaV247(),c=p.collection.find(x=>String(x.characterId)===String(id));if(!c)return;const s=new Set(c.folderIds||[]);s.has(fid)?s.delete(fid):s.add(fid);c.folderIds=[...s];saveData();openCardFolderModalV247(id);renderCollectionV16()}
function deleteCardFolderV247(fid){const p=ensureCollectionMetaV247();p.cardFoldersV247=p.cardFoldersV247.filter(f=>f.id!==fid);for(const c of p.collection||[])c.folderIds=(c.folderIds||[]).filter(x=>x!==fid);if(p.collectionUiV247.folder===fid)p.collectionUiV247.folder='all';saveData();openCardFolderModalV247();renderCollectionV16()}
function setCardFolderFilterV247(fid){collectionUiV247().folder=fid;closeCardFolderModalV247();saveData();renderCollectionV16()}
function toggleCardHiddenV247(id){const p=ensureCollectionMetaV247(),c=p.collection.find(x=>String(x.characterId)===String(id));if(!c)return;c.hidden=!c.hidden;saveData();renderCollectionV16();profileToastV16(c.hidden?'Карточка скрыта':'Карточка возвращена',c.name)}
Object.assign(window,{openCardFolderModalV247,closeCardFolderModalV247,createCardFolderV247,toggleCardFolderV247,deleteCardFolderV247,setCardFolderFilterV247,toggleCardHiddenV247});
setCollectionFilterV166=function(filter){collectionFilterV166=['all','showcase','hidden'].includes(filter)?filter:'all';renderCollectionV16()};window.setCollectionFilterV166=setCollectionFilterV166;
renderCollectionV16=function(){const p=ensureCollectionMetaV247();ensureCardProfileV205();renderAnimeTicketsV205();const box=$('#profileCollection');if(!box)return;populateCollectionSelectsV247();const ui=p.collectionUiV247,s=$('#collectionSearchV247'),rr=$('#collectionRarityV247'),sort=$('#collectionSortV247');if(s&&document.activeElement!==s)s.value=ui.query||'';if(rr)rr.value=ui.rarity||'all';if(sort)sort.value=ui.sort||'new';document.querySelectorAll('#profileCollectionFilters [data-card-filter]').forEach(b=>b.classList.toggle('active',b.dataset.cardFilter===collectionFilterV166));let cards=[...p.collection];if(collectionFilterV166==='all')cards=cards.filter(c=>!c.hidden);if(collectionFilterV166==='showcase')cards=cards.filter(c=>c.showcase&&!c.hidden);if(collectionFilterV166==='hidden')cards=cards.filter(c=>c.hidden);const q=String(ui.query||'').trim().toLowerCase();if(q)cards=cards.filter(c=>`${c.name||''} ${c.animeTitle||''}`.toLowerCase().includes(q));if(ui.rarity!=='all')cards=cards.filter(c=>String(c.rarity||'').toLowerCase()===ui.rarity);if(ui.anime!=='all')cards=cards.filter(c=>String(c.animeTitle||'')===ui.anime);if(ui.folder!=='all')cards=cards.filter(c=>(c.folderIds||[]).includes(ui.folder));cards.sort((a,b)=>ui.sort==='name'?String(a.name||'').localeCompare(String(b.name||''),'ru'):ui.sort==='anime'?String(a.animeTitle||'').localeCompare(String(b.animeTitle||''),'ru'):ui.sort==='rarity'?(cardRarityRankV247(b.rarity)-cardRarityRankV247(a.rarity)||String(a.name||'').localeCompare(String(b.name||''),'ru')):(Number(b.at)||0)-(Number(a.at)||0));box.classList.toggle('compact-v247',!!ui.compact);$('#collectionCompactBtnV247')?.classList.toggle('active',!!ui.compact);if(!p.collection.length){box.innerHTML='<div class="profile-empty">Коллекция пока пустая.</div>';return}if(!cards.length){box.innerHTML='<div class="profile-empty">По этим фильтрам ничего не нашлось.</div>';return}box.innerHTML=cards.map(c=>{const vote=c.waifuVote||ensureWaifuTasteV183().votes[String(c.characterId)]||'',rate=c.ticketType==='waifu'?`<div class="waifu-rate-v183"><button class="${vote==='love'?'active love':''}" onclick="setWaifuVoteV183(${Number(c.characterId)||0},'love')">💘</button><button class="${vote==='ok'?'active ok':''}" onclick="setWaifuVoteV183(${Number(c.characterId)||0},'ok')">🙂</button><button class="${vote==='no'?'active no':''}" onclick="setWaifuVoteV183(${Number(c.characterId)||0},'no')">🚫</button></div>`:'',type=c.ticketType==='waifu'?'<span class="collection-ticket-type">💗 WAIFU</span>':c.ticketType==='anime'?'<span class="collection-ticket-type anime">🎟️ TITLE</span>':'',folders=(c.folderIds||[]).map(id=>p.cardFoldersV247.find(f=>f.id===id)?.name).filter(Boolean);return `<article class="collection-card ${c.ticketType==='waifu'?'ticket-waifu':''} ${c.showcase?'in-showcase':''}" data-character-id="${Number(c.characterId)||0}"><span class="collection-rarity ${rarityClassV16(c.rarity)}">${esc(String(c.rarity).toUpperCase())}</span>${type}<img src="${esc(c.image||'')}" alt="${esc(c.name)}"><div class="collection-card-copy"><strong>${esc(c.name)}</strong><small>${esc(c.animeTitle||'')}</small>${folders.length?`<div class="card-folder-chips-v247">${folders.slice(0,3).map(x=>`<span>📁 ${esc(x)}</span>`).join('')}</div>`:''}<div class="collection-global-v205">🌐 выбито: <b>${formatGlobalPullsV205(c.globalPulls)}</b></div>${rate}<div class="collection-card-actions"><button onclick="openAvatarStudioV205(${Number(c.characterId)||0})">🙂 Аватар</button><button onclick="openCardFolderModalV247(${Number(c.characterId)||0})">📁</button><button class="${c.showcase?'showcase-on':''}" onclick="toggleCardShowcaseV166(${Number(c.characterId)||0})">${c.showcase?'★':'☆'}</button><button onclick="toggleCardHiddenV247(${Number(c.characterId)||0})">${c.hidden?'↩':'⌫'}</button></div></div></article>`}).join('');setTimeout(()=>refreshGlobalPullCountsV205(),0);setTimeout(()=>syncCollectionRaritiesV205(),50)};
function bindCollectionToolsV247(){const ui=collectionUiV247();if($('#collectionSearchV247')?.dataset.boundV247)return;const s=$('#collectionSearchV247');if(s){s.dataset.boundV247='1';s.oninput=e=>{ui.query=e.target.value;clearTimeout(bindCollectionToolsV247.t);bindCollectionToolsV247.t=setTimeout(()=>{saveData();renderCollectionV16()},130)}};$('#collectionRarityV247').onchange=e=>{ui.rarity=e.target.value;saveData();renderCollectionV16()};$('#collectionAnimeV247').onchange=e=>{ui.anime=e.target.value;saveData();renderCollectionV16()};$('#collectionFolderV247').onchange=e=>{ui.folder=e.target.value;saveData();renderCollectionV16()};$('#collectionSortV247').onchange=e=>{ui.sort=e.target.value;saveData();renderCollectionV16()};$('#collectionFoldersBtnV247').onclick=()=>openCardFolderModalV247();$('#collectionCompactBtnV247').onclick=()=>{ui.compact=!ui.compact;saveData();renderCollectionV16()}}
const renderProfileV16BeforeCollectionToolsV247=renderProfileV16;renderProfileV16=function(){renderProfileV16BeforeCollectionToolsV247();setTimeout(bindCollectionToolsV247,20)};

/* More themed tickets for actually finishing a title. */
function themedTicketRewardCountV247(e){if(String(e?.format||'').toUpperCase()==='MOVIE')return 3;let eps=Number(e?.episodes)||0;if(!eps){try{eps=(watchPartsV15(e)||[]).reduce((n,p)=>n+(Number(p?.episodes)||0),0)}catch{}}if(eps<=13)return 3;if(eps<=26)return 5;if(eps<=50)return 7;if(eps<=100)return 10;return 12}
const verifyEpisodeBeforeRewardsV247=verifyEpisodeV16;
...
const TICKET_SHOP_V247=[{id:'char1',icon:'🎴',name:'Character Ticket',desc:'1 обычный билет персонажа',price:180,tickets:1,waifu:0},{id:'char5',icon:'🎴',name:'Character Pack ×5',desc:'Пять обычных билетов',price:780,tickets:5,waifu:0,best:true},{id:'char10',icon:'🎴',name:'Character Pack ×10',desc:'Десять билетов ещё выгоднее',price:1450,tickets:10,waifu:0},{id:'waifu1',icon:'💗',name:'Waifu Ticket',desc:'1 специальный билет героини',price:420,tickets:0,waifu:1},{id:'waifu3',icon:'💗',name:'Waifu Pack ×3',desc:'Три специальных билета',price:1100,tickets:0,waifu:3,best:true}];
function ticketFinalPriceV247(item){return Math.max(1,Math.round(item.price*(profilePremiumV168()?.8:1)))}
buyTicketPackV168=function(id){const p=ensureProfileV16(),item=TICKET_SHOP_V247.find(x=>x.id===id);if(!item)return;const price=ticketFinalPriceV247(item);if(p.stars<price){profileToastV16('Не хватает Stars',`Нужно ${price} ✦ · у тебя ${p.stars} ✦`);return}p.stars-=price;if(item.tickets)addTicketV16(item.tickets,`Магазин · ${item.name}`,true);if(item.waifu)addWaifuTicketV165(item.waifu,`Магазин · ${item.name}`,true);profileActivityV16('🛒',`Куплено: ${item.name}`,`−${price} ✦`);saveData();renderProfileV16();profileToastV16(`${item.icon} ${item.name}`,`−${price} ✦${profilePremiumV168()?' · Premium −20%':''}`)};window.buyTicketPackV168=buyTicketPackV168;
renderShopV16=function(){const p=ensureProfileV16(),box=$('#profileShop');if(!box)return;const premium=profilePremiumV168(),ticketHtml=`<section class="ticket-store-v168"><div class="ticket-store-head-v168"><div><span>БИЛЕТЫ ЗА STARS</span><h3>Открывай карточки чаще</h3><p>Цены снижены. Premium даёт ещё −20% в магазине, но не влияет на лидерборды.</p></div><div class="premium-star-chip-v168 ${premium?'active':''}">👑 ${premium?'Premium · −20%':'Premium · скидка −20%'}</div></div><div class="ticket-store-grid-v168">${TICKET_SHOP_V247.map(i=>{const price=ticketFinalPriceV247(i);return `<article class="ticket-shop-card-v168 ${i.waifu?'waifu':''} ${i.best?'best':''}">${i.best?'<b class="ticket-best-v168">ВЫГОДНЕЕ</b>':''}<div class="ticket-shop-icon-v168">${i.icon}</div><div><h4>${esc(i.name)}</h4><p>${esc(i.desc)}</p></div><div class="ticket-shop-buy-v168"><strong>✦ ${price}${premium?` <s>${i.price}</s>`:''}</strong><button onclick="buyTicketPackV168('${i.id}')">Купить</button></div></article>`}).join('')}</div></section>`,cosmetics=PROFILE_SHOP_V16.map(item=>{const owned=p.ownedCosmetics.includes(item.id),equipped=p.equipped.frame===item.className;return `<article class="shop-card"><div class="shop-preview"><i class="${item.className}"></i></div><h4>${esc(item.name)}</h4><p>${esc(item.desc)}</p><div class="shop-buy-row"><span class="shop-price">${owned?'Куплено':`✦ ${item.price}`}</span><button class="${owned?'secondary':''}" onclick="buyOrEquipCosmeticV16('${item.id}')">${equipped?'✓ Надето':owned?'Надеть':'Купить'}</button></div></article>`}).join('');box.innerHTML=`${ticketHtml}<div class="shop-section-title-v168"><span>КОСМЕТИКА</span><h3>Оформление профиля</h3></div><div class="cosmetic-grid-v168">${cosmetics}</div>`};

function updateSidebarCloudV247(){const s=document.querySelector('.sidebar-status .sidebar-label-text'),d=document.querySelector('.sidebar-status .status-dot');if(s)s.textContent=senimeSignedV247()?'Senime Cloud':'Гостевой режим';if(d)d.classList.toggle('offline-v247',!senimeSignedV247())}
setTimeout(()=>{updateSidebarCloudV247();loadNotificationBadgeV247();if(senimeSignedV247())socialSyncPublicProfileV242?.().catch(()=>{})},1000);window.addEventListener('focus',()=>{updateSidebarCloudV247();loadNotificationBadgeV247()});
console.info(`Senime V${SENIME_V247}: Community Center + leaderboards + user collections + social comments + collection organizer loaded`);

/* V24.7 final wiring/regression guards. */
loadCommunityFriendsV247=async function(){
  const box=$('#communityFriendsV247');if(!box)return;box.innerHTML='<div class="community-loading-v247">Загружаю друзей…</div>';
  try{await socialLoadV241();const me=senimeMeV247(),friends=socialStateV241.friends.map(id=>socialStateV241.profiles.get(id)).filter(Boolean),req=(socialStateV241.requests||[]).filter(r=>r.receiver_id===me).map(r=>({r,p:socialStateV241.profiles.get(r.sender_id)})).filter(x=>x.p);
    box.innerHTML=`<div class="friends-page-v247"><header class="community-page-head-v247"><div><span>PEOPLE</span><h2>Друзья и подписки</h2><p>Заявки, подписки и поиск людей — отдельно от профиля.</p></div><div class="friends-kpis-v247"><b>${friends.length}<small>друзей</small></b><b>${socialStateV241.following.length}<small>подписок</small></b><b>${socialStateV241.followers.length}<small>подписчиков</small></b></div></header><div class="community-search-v247"><input id="communityFriendSearchV247" placeholder="@handle или ник"><button id="communityFriendSearchBtnV247">Найти</button></div><div id="communityFriendResultsV247"></div>${req.length?`<section class="community-section-v247"><h3>Заявки <i>${req.length}</i></h3><div class="community-people-grid-v247">${req.map(x=>socialPersonCardV241(x.p,{context:'request',requestId:x.r.id})).join('')}</div></section>`:''}<section class="community-section-v247"><h3>Друзья <i>${friends.length}</i></h3><div class="community-people-grid-v247">${friends.length?friends.map(p=>socialPersonCardV241(p,{context:'friend'})).join(''):'<div class="community-empty-v247">Пока никого.</div>'}</div></section></div>`;
    $('#communityFriendSearchBtnV247').onclick=communitySearchPeopleV247;$('#communityFriendSearchV247').onkeydown=e=>{if(e.key==='Enter')communitySearchPeopleV247()};
  }catch(e){box.innerHTML=`<div class="community-empty-v247">${esc(e.message||String(e))}</div>`}
};

/* The old share button captured the pre-V24.7 function at boot: replace it once. */
(function rebindShareV247(){const old=$('#detailShareAnime');if(!old||old.dataset.v247)return;const b=old.cloneNode(true);b.dataset.v247='1';old.replaceWith(b);b.onclick=shareOpenedAnimeV247})();

/* Keep collection toolbar bindings alive after profile rerenders. */
setTimeout(()=>{bindCollectionToolsV247();renderCollectionV16?.()},500);

/* Refresh social pages after common actions. */
const socialToggleFollowBeforeV247=socialToggleFollowV241;
socialToggleFollowV241=async function(...a){const r=await socialToggleFollowBeforeV247(...a);if(communityV247.tab==='friends'&&!$('#communityModalV247')?.classList.contains('hidden'))loadCommunityFriendsV247();return r};window.socialToggleFollowV241=socialToggleFollowV241;
const socialRespondBeforeV247=socialRespondV241;
socialRespondV241=async function(...a){const r=await socialRespondBeforeV247(...a);if(communityV247.tab==='friends'&&!$('#communityModalV247')?.classList.contains('hidden'))loadCommunityFriendsV247();return r};window.socialRespondV241=socialRespondV241;
const socialRemoveFriendBeforeV247=socialRemoveFriendV241;
socialRemoveFriendV241=async function(...a){const r=await socialRemoveFriendBeforeV247(...a);if(communityV247.tab==='friends'&&!$('#communityModalV247')?.classList.contains('hidden'))loadCommunityFriendsV247();return r};window.socialRemoveFriendV241=socialRemoveFriendV241;
...
/* V24.7.2 compatibility: V24.7 uses attrV247(); keep both names valid. */
function v247EscAttr(s){ return attrV247(s); }
