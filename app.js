...
async function querySubdlV19(){const cfg=providerConfigV19();if(!cfg.subdlKey)throw new Error('нет API key');const part=currentPartV19(),season=watchStateV15.season+1,episode=watchStateV15.episode,titles=[part?.title,watchStateV15.entry?.title].filter(Boolean);let last=null;for(const title of [...new Set(titles)]){try{const u=new URL(WATCH_SUBDL_API_V19);u.searchParams.set('api_key',cfg.subdlKey);u.searchParams.set('film_name',title);u.searchParams.set('type','tv');u.searchParams.set('season_number',String(season));u.searchParams.set('episode_number',String(episode));u.searchParams.set('languages',cfg.subLangs||'RU,EN');u.searchParams.set('subs_per_page','30');u.searchParams.set('unpack','1');u.searchParams.set('client','custom_integration');if(part?.year)u.searchParams.set('year',String(part.year));const data=await fetchJsonTimeoutV19(u.toString(),{},12000);if(!data?.status)throw new Error(data?.error||'SubDL search error');const out=[];for(const s of data.subtitles||[]){const files=Array.isArray(s.unpack_files)?s.unpack_files:[];for(const f of files){const fe=Number(f.episode||s.episode||0);if(fe&&fe!==Number(episode))continue;const url=absSubdlUrlV19(f.url);if(!url)continue;const lang=String(f.language||s.language||'').toUpperCase();out.push({url,id:`subdl_${lang}_${f.file_n_id||Math.random().toString(36).slice(2)}`,label:`${subtitleLangLabelV19(lang)} · ${String(f.release_name||s.release_name||'SubDL').slice(0,42)}`,provider:'subdl',lang,format:f.format||'srt'})}}if(out.length)return out}catch(e){last=e}}throw last||new Error('субтитры не найдены')}
async function ensureSubdlV19({force=false}={}){const cfg=providerConfigV19();if(cfg.autoSubtitles===false||!cfg.subdlKey||watchResolverV19.subdlBusy)return false;const key=subdlKeyV19(),cache=subdlCacheV19();if(!force&&cache[key]&&Date.now()-Number(cache[key].at||0)<1000*60*60*24*7){installSubdlTracksV19(cache[key].tracks||[]);return !!cache[key].tracks?.length}watchResolverV19.subdlBusy=true;try{const tracks=await querySubdlV19();cache[key]={at:Date.now(),tracks};saveSubdlCacheV19(cache);installSubdlTracksV19(tracks);watchResolverV19.lastSubdlError='';return !!tracks.length}catch(e){watchResolverV19.lastSubdlError=String(e?.message||e);cache[key]={at:Date.now(),tracks:[]};saveSubdlCacheV19(cache);return false}finally{watchResolverV19.subdlBusy=false}}
...
async function resolveProviderV19(opts={}){const mode=watchProviderV17(),cfg=providerConfigV19();if(mode==='direct')return customDirectSourcesV19().length?'direct':'none';if(mode==='embed')return customEmbedSourcesV19().length?'embed':'none';if(mode==='aniliberty'){const ok=await ensureAniLibertyEpisodeV171({force:!!opts.forceResolver||!!opts.forceAniLiberty});return ok?'aniliberty':'none'}if(mode==='kodik'){const ok=await ensureKodikEpisodeV19({force:!!opts.forceResolver});return ok?'kodik':'none'}if(currentLocalSourcesV171().length)return 'direct';if(cfg.preferNative!==false){if(await ensureAniLibertyEpisodeV171({force:!!opts.forceResolver||!!opts.forceAniLiberty}))return 'aniliberty';if(cfg.kodikToken&&await ensureKodikEpisodeV19({force:!!opts.forceResolver}))return 'kodik'}else{if(cfg.kodikToken&&await ensureKodikEpisodeV19({force:!!opts.forceResolver}))return 'kodik';if(await ensureAniLibertyEpisodeV171({force:!!opts.forceResolver||!!opts.forceAniLiberty}))return 'aniliberty'}if(customDirectSourcesV19().length)return 'direct';if(customEmbedSourcesV19().length)return 'embed';return 'none'}
